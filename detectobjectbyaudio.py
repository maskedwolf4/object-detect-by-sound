...
#load the classes names and store in list
classesNames = []
df = pd.read_csv("CV project1/classes (1).txt", header=None, names=["ClassName"])
for index , row in df.iterrows():
    ClassName = df.iloc[index]['ClassName']
    classesNames.append(ClassName)

 #camera access and setting
...
def getTextFromAudio():
    ...
    data , samplerate = sf.read(audioFileName)
    sf.write('c:/temp/outputNew.wav', data , samplerate, subtype='PCM_16')
...
while True:
    ...
    # draw a "Button"
    cv2.rectangle(frame , (x1,y1),(x2,y2), (153,30,30), -1 ) #-1 is filled cretangle
